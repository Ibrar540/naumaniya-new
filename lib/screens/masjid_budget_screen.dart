...
class MasjidBudgetScreen extends StatefulWidget {
  ...
  @override
  State<MasjidBudgetScreen> createState() => _MasjidBudgetManagementScreenState();
}

class _MasjidBudgetManagementScreenState extends State<MasjidBudgetScreen> {
  ...
  Future<void> _createSection() async {
    ...
    await _budgetProvider.addSection(Section(name: name, institution: 'masjid', type: _selectedType ?? 'income'));
  ...
  Future<void> _downloadPdf() async {
    ...
    try {
              ...
              children: [
                pw.Text(
                  languageProvider.isUrdu 
                    ? 'مسجد - ${_selectedSection?.name} - ${_selectedType == 'income' ? 'آمدنی' : 'خرچ'}'
                    : 'Masjid - ${_selectedSection?.name} - ${_selectedType == 'income' ? 'Income' : 'Expenditure'}',
                  style: pw.TextStyle(
                    fontSize: 24,
      ...
      final fileName = 'Masjid_${_selectedSection?.name}_${_selectedType}_${DateTime.now().millisecondsSinceEpoch}.pdf';
  ...
  void _downloadExcel() async {
    ...
    // Web download
    final fileName = 'Masjid_${_selectedSection?.name}_${_selectedType}_${DateTime.now().millisecondsSinceEpoch}.xlsx';
          ...
          Expanded(
              ...
              child: SingleChildScrollView(
                                ...
                                child: DataTable(
                                  ...
                                  rows: _filteredSectionData.map((row) => DataRow(
                                    cells: (languageProvider.isUrdu
                                      ? [
