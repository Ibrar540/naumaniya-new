...
class TeacherEnterDataScreen extends StatefulWidget {
  ...
  const TeacherEnterDataScreen({Key? key, this.teacher}) : super(key: key);
...
class _TeacherEnterDataScreenState extends State<TeacherEnterDataScreen> {
  ...
  Future<void> _addTeacher() async {
    ...
    String status = (_selectedStatus == null || _selectedStatus!.isEmpty) ? 'Active' : _selectedStatus!;
    ...
    try {
      final teacher = Teacher(
        ...
        salary: salary,
        status: statusToSave,
        leavingDate: _leavingDate != null ? DateFormat('yyyy-MM-dd').format(_leavingDate!) : '',
      );

      await _teacherProvider.addTeacher(teacher);
      final languageProvider = Provider.of<LanguageProvider>(context, listen: false);
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(languageProvider.getText('teacher_added_successfully')),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${languageProvider.getText('error_adding_teacher')}: $e'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(languageProvider.isUrdu ? 'اساتذہ - ڈیٹا داخل کریں' : 'Teachers - Enter Data'),
                    ...
                    child: Container(
                      ...
                      child: Column(
                        children: [
                          ...
                          SizedBox(height: 16),
                          Text(
                            languageProvider.isUrdu ? 'نیا استاد شامل کریں' : 'Add New Teacher',
                            style: TextStyle(
                              fontSize: 24,
                      ...
                      child: Column(
                        children: [
                          ...
                          _buildTextField(
                            ...
                            hint: languageProvider.getText('enter_teacher_name'),
                            icon: Icons.person,
                            validator: (value) {
                              if (value == null || value.trim().isEmpty) {
                                return languageProvider.getText('please_enter_teacher_name');
                              }
                              return null;
                            },
                          ...
                          _buildTextField(
                            ...
                            label: languageProvider.getText('mobile_number'),
                            hint: languageProvider.getText('enter_mobile_number'),
                            icon: Icons.phone,
                          ...
                          if (_selectedStatus == 'Left')
                            Row(
                              children: [
                                Text(languageProvider.getText('leaving_date')),
                                TextButton(
                                  onPressed: _pickLeavingDate,
                                  child: Text(_leavingDate != null ? DateFormat('yyyy-MM-dd').format(_leavingDate!) : languageProvider.getText('select_date')),
                                ),
                              ],
                            ),
                          SizedBox(height: 20),
                              ...
                              child: _isLoading
                                ...
                                : Row(
                                    ...
                                    children: [
                                      ...
                                      SizedBox(width: 8),
                                      Text(
                                        languageProvider.getText('add_teacher'),
                                        style: TextStyle(
                                          fontSize: 16,
                                          fontWeight: FontWeight.bold,
                                        ),
